...
(function () {
	...
	$scope.showDivAnswer = function (index,paramQuestion)
		...
		$scope.hiddenDiv[paramQuestion._id] = !$scope.hiddenDiv[paramQuestion._id];
		var myQuestionId = paramQuestion._id;
		console.log('the myQuestionId is ' + myQuestionId);
		var myAnswerCount = paramQuestion.question.answers.length;
...
//$scope.offices = [];
		...
		$scope.myLoginShow = function() {
		 ...
		 console.log('the paramAnwer id is....'+paramQuestion._id);
        
		 $scope.showContentAnswers[paramQuestion._id] = !$scope.showContentAnswers[paramQuestion._id];
  ...
  $scope.addQuestion = function (createdByParam) {
    ...
    if ($scope.formData.questionText !== "") {
		...
		$http.post('/MakeSurvey/addQuestion',{questionText:$scope.formData.questionText,createdByParam:createdByParam})
		   ...
		   .success(function(data) {
	        ...
	        //$scope.questions.concat(myNewQuestionArray,myNewAnswerSpaceArray);
			
	        //console.log('$scope.questions after addQuestions is ....'  + JSON.stringify($scope.questions));
    ...
    console.log('the passed in question.questions.answers in scope.addAnswer is..' + JSON.stringify(question.question.answers) );

	if ($scope.formData.answerText !== "") {
	    
		$scope.formData.questionId = question._id;

		console.log('the $scope.formData is ...'  + JSON.stringify($scope.formData ) );
		console.log('the $scope.survey-questionId is ...'  + question._id );
		...
		$http.post('/MakeSurvey/addAnswer',$scope.formData)
		   ...
		   .success(function(data) {
			...
			question.question.answers.push(angular.fromJson(data.answer));
			
			//question.question.answers.push($scope.formData.answerText[question._id]);
			
			//question.question.answers.push('tesasasat');
			
			
			console.log(' ZZZZZ the $scope.formData.answerText[question.id].ZZZZ' + $scope.formData.answerText[question._id] );
			
			
			$scope.formData.answerText = "";
			
			console.log(' !!!! the $scope.formData.answerText[question.id].!!!' + $scope.formData.answerText[question._id] );
...
// ******************* for adding todos ***************************
	...
	$scope.deleteQuestion = function(question,questions){
		...
		var indexQuestion;
		indexQuestion = findWithAttr($scope.questions.questions,'_id', question._id); // returns 0
		...
		console.log( '..$scope.deleteQuestion..');
		    ...
		    $http.delete('/MakeSurvey/deleteQuestion/' + question._id, {params: {questionId:question._id}})
			.success(function(data) {
	...
	console.log('the passed in question._id in deleteAnswer is ' + JSON.stringify(question._id));
	...
	var indexAnswer = question.question.answers.indexOf(answer);
	question.question.answers.splice(indexAnswer,1);
			
     console.log( '..$scope.deleteAnswer..');
			
			
	    $http.delete('/MakeSurvey/deleteAnswer/' + answer.auid + '/' + question._id, {params: {answerId:answer.auid,questionId:question._id}})
		
		
		//$http.put('/MakeSurvey/updateQuestion/' + question._id + '/' + question.question.questionText, {params: {questionId:question._id, questionText:question.question.questionText}})
...
//CORRECT updateAnswer		
  ...
  // server api router.put( '/updateAnswer/:answerId/:answerText'
   ...
   console.log('the indexAnswer in updateAnswer is ' + indexAnswer);   
	
    console.log( '..$scope.deleteAnswer..');
			
	
	$http.put('/MakeSurvey/updateAnswer/' + question._id +'/' + indexAnswer + '/' + answer.answerText, {params: {questionId:question._id, answerId:indexAnswer, answerText:answer.answerText}})
		.success(function(data) {
   ...
   $scope.updateQuestion = function(question){
	...
	console.log('the passed in question.id in updateQuestion is ' + JSON.stringify(question._id));
