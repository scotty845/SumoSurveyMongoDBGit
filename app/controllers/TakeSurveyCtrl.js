...
(function () {
    ...
    var TakeSurveyCtrl = function ($scope, $http,genericServices,$window,$location,$anchorScroll)
    {
        ...
        var refresh = function(){
			...
			$scope.questions = data.question[0];
			console.log('the $scope.questions is');
	...
	//
    $scope.save= function(answer,question) {
		 ...
		 if ($scope.genericServices.isEmptyObjectService(answer) !== true) {
			...
			//$http.post('/TakeSurvey/saveSurveyAnswer', answer,question)
			
			$http.post('/TakeSurvey/saveSurveyAnswer', {answer:answer.answer,question:question._id})
